...
if ( ! function_exists('disco'))
{
     ...
     * @return string
     */
    function disco($licencia)
    {
        $disco = Config1::where("licencia_id",$licencia)->pluck("disco")->first();
        $ruta = '';
        if($disco){
            $ruta = Disco::where("disco", $disco[0])->pluck("ruta")->first();
        }
        return $ruta;
